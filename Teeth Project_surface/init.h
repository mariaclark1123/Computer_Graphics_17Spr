...
void setcurve(char* filename, SB* slice)
{
	int numVertex = 0;
	int numFaces = 0;
	...
	while (!feof(fp))
	{
		...
		if (line[0] == 'v' && line[1] == ' ')
		{
			...
			slice->vertices[numVertex][X] = (x + 22) * 30 + 200;
			slice->vertices[numVertex][Y] = (y - 78) * 30 + 200;
			slice->vertices[numVertex][Z] = (z - 38) * 30;
			numVertex++;
		}
	...
	x_max = x_min = slice->vertices[0][X];
	y_max = y_min = slice->vertices[0][Y]; 
	...
	for (int i = 0; i < 4; i++)
	{
		...
		SET_PT2(slice->cur[i].control_pts[0], start[i][0], start[i][1]);
		SET_PT2(slice->cur[i].control_pts[1], ctrlp_2[i][0], ctrlp_2[i][1]);
		SET_PT2(slice->cur[i].control_pts[2], ctrlp_3[i][0], ctrlp_3[i][1]);
		SET_PT2(slice->cur[i].control_pts[3], end[i][0], end[i][1]);
	}
